...
export const FocusProvider: React.FC<{
  ...
  userId: string;
  themeColor?: string;
}> = ({ children, userId, themeColor = "blue" }) => {
  const getFocusKey = useCallback(
    (key: string) => `focus_${key}_${userId}`,
    [userId]
  );
  ...
  const [timerType, setTimerType] = useState<TimerType>(
    () =>
      (localStorage.getItem(`focus_timerType_${userId}`) as TimerType | null) ||
      "pomodoro"
  );
  const [mode, setModeState] = useState<"stopwatch" | "pomodoro">(
    () =>
      (localStorage.getItem(`focus_mode_${userId}`) as
        | "stopwatch"
        | "pomodoro"
        | null) || "stopwatch"
  );
  ...
  const lastTickRef = useRef<number>(
    parseFloat(localStorage.getItem(`focus_lastTick_${userId}`) || "0")
  );
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // Persist helpers
  useEffect(
    () => localStorage.setItem(getFocusKey("timerType"), timerType),
    [timerType]
  );
  useEffect(() => localStorage.setItem(getFocusKey("mode"), mode), [mode]);
  useEffect(
    () => localStorage.setItem(getFocusKey("duration"), duration.toString()),
    [duration]
  );
  useEffect(
    () => localStorage.setItem(getFocusKey("elapsed"), elapsed.toString()),
    [elapsed]
  );
  useEffect(
    () => localStorage.setItem(getFocusKey("running"), String(isRunning)),
    [isRunning]
  );
  useEffect(() => {
    if (sessionTitle)
      localStorage.setItem(getFocusKey("sessionTitle"), sessionTitle);
  ...
  useEffect(() => {
    ...
    } else if (!Capacitor.isNativePlatform() && !isRunning) {
  ...
  useEffect(() => {
    const savedTick = parseFloat(
      localStorage.getItem(getFocusKey("lastTick")) || "0"
    );
    ...
    if (wasRunning && savedTick > 0) {
      ...
      if (delta > 0 && delta < 86400) {
        // If delta < 1 day, assume valid catch-up
  ...
  const startPomodoro = (title: string, durationMinutes: number) => {
    ...
    const secondsRemaining = Math.max(0, durationMinutes * 60 - (title !== sessionTitle ? 0 : elapsed));
    scheduleFinishNotification(secondsRemaining);

    if (Capacitor.isNativePlatform()) {
      console.log(
        "[FocusContext] Calling OverlayTimer.startTimer for pomodoro", secondsRemaining
      );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("start_widget_timer", handleWidgetStart);
  }, [isRunning]);
  ...
  useEffect(() => {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      OverlayTimer.addListener("timerTick", (info: any) => {
        // Update React UI from native tick
        const time = info.time as number;
        setElapsed((prev) => {
          const { mode, duration } = stateRef.current;
          if (mode === "pomodoro") {
            // For pomodoro, time is seconds remaining
            const calculatedElapsed = duration * 60 - time;
            return calculatedElapsed >= 0 ? calculatedElapsed : 0;
          } else {
            // For subject/stopwatch, time is elapsed seconds
            return time;
          }
        });
      }).catch(console.error);
