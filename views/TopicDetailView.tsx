...
export const TopicDetailView: React.FC<TopicDetailViewProps> = React.memo(({ topic, userId, navigateTo, onUpdateTopic, themeColor }) => {
    ...
    const alreadyCompletedToday = topic?.lastCompletedDate === today;
    const isReadyForReview = !alreadyCompletedToday && (
        lastRepetition
            ? new Date(lastRepetition.nextReviewDate) <= new Date()
            : true
    );
                        ...
                        <div className="flex space-x-2 mt-auto">
                            <button
                                ...
                                disabled={
                                    !isQuizUnlocked || 
                                    saveStatus === 'saving' || 
                                    (!isReadyForReview && !topic.isJourneyPaused) || 
                                    isLoadingBody || 
                                    !!currentJob || 
                                    topic.isJourneyPaused ||
